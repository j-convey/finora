...
class SubscriptionsPage extends ConsumerWidget {
              ...
              SizedBox(
                ...
                child: FilledButton(
                  onPressed: () async {
                    ...
                    try {
                      if (isEditing) {
                        await ref.read(subscriptionsProvider.notifier).update(
                              existing.id,
  ...
  void _showTransactionPicker(BuildContext context, WidgetRef ref) {
    final transactions = ref.read(transactionsProvider);
    
    // Filter to expenses and income only (not transfers)
    final selectableTransactions = transactions
    ...
    if (selectableTransactions.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('No transactions available')),
      );
      return;
    }
          ...
          children: [
            Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    'Select a transaction to use as a template',
            ...
            Expanded(
              child: ListView.builder(
                ...
                itemCount: selectableTransactions.length,
                itemBuilder: (context, index) {
                  final transaction = selectableTransactions[index];
                  return ListTile(
                    title: Text(transaction.title),
                    subtitle: Text(transaction.merchantName ?? transaction.category),
                    trailing: Text(
                    ...
                    onTap: () {
                      ...
                      _showSubscriptionSheetFromTransaction(context, ref, transaction);
                    },
                  );
                },
              ),
  ...
  void _showSubscriptionSheetFromTransaction(
    BuildContext context,
    WidgetRef ref,
    TransactionModel transaction,
  ) {
    // Pre-fill form from transaction
    final nameController = TextEditingController(
      text: transaction.merchantName ?? transaction.title,
    );
    final merchantController = TextEditingController(text: transaction.merchantName);
    final categoryController = TextEditingController(text: transaction.category);
    final expectedAmountController = TextEditingController(
      text: transaction.amount.toStringAsFixed(2),
    );
    final minAmountController = TextEditingController(
      text: (transaction.amount * 0.9).toStringAsFixed(2), // 90% of amount
    );
    final maxAmountController = TextEditingController(
      text: (transaction.amount * 1.1).toStringAsFixed(2), // 110% of amount
    );
    final matchingNotesController = TextEditingController();

    var selectedStatus = SubscriptionStatus.active;
    var selectedRecurrenceUnit = RecurrenceUnit.month;
    var selectedRecurrenceInterval = 1;
    var autoLinkEnabled = true;
        ...
        builder: (context, setStateSheet) => SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 8),
              Text(
                'Transaction: ${transaction.title} on ${transaction.date.toString().split(' ')[0]}',
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          Text(
            'Track recurring charges and income',
            style: tt.bodyMedium,
          ),
          ...
          children: [
            ...
            Row(
              children: [
                ...
                Chip(
                  label: Text(
                    'Every ${subscription.recurrenceLabel}',
                  ),
